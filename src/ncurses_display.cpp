...
// 50 bars uniformly displayed from 0 - 1
// 2% is one bar(|)
...
void NCursesDisplay::DisplayProcesses(std::vector<Process>& processes,
                                      WINDOW* window, int numOfProcesses) {
  int row{0};
...
void NCursesDisplay::Display(System& system, int numOfProcesses) {
  ...
  WINDOW* process_window =
      newwin(3 + numOfProcesses, x_max - 1, system_window->_maxy + 1, 0);
  ...
  while (1) {
    ...
    DisplayProcesses(system.Processes(), process_window, numOfProcesses);
